...
The default protocol for DigitalOcean Load Balancers. Options are `tcp`, `udp`, `http`, `https`, and `http2`. Defaults to `tcp`. Note that if UDP is supported a healtcheck configuration configured against a TCP/HTTP/HTTPS service is still required. In order for UDP configuration, please reach out to support with a ticket to enable it.
