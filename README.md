...
When you are creating load-balancers through CCM (via `LoadBalancer`-typed Services), it is very much important that you **must not change the DO load-balancer configuration manually.** Such changes will eventually be reverted by the reconciliation loop built into CCM. There will be one exception which is load-balancer names. It can be changed (see also [the documentation on load-balancer ID annotations](/docs/getting-started.md#load-balancer-id-annotations)).
